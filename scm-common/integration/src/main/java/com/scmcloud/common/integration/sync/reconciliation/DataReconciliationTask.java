...
public class DataReconciliationTask {
    ...
    public void reconcile() {
        ...
        log.debug("[Reconciliation] Starting data reconciliation task...");
        ...
        log.debug("[Reconciliation] Completed in {}ms - total checked: {}, fixed: {}, failed: {}",
    ...
    private ReconciliationResult reconcileAggregate(String aggregateType, DataSyncHandler handler) {
        ...
        // 2. 从目标库获取冗余数据
        // 3. 比对差异
        // 4. 如果启用自动修复，调handler.fullSync() 修复

        // 由于具体的对账逻辑依赖业务表结构，
        // 这里只提供框架，具体实现Handler 自己负责
