...
public class ShardingMetricsMonitor {
    ...
    /**
     * 每分钟采集一�ShardingSphere 内部元数据统计信�
     */
    @Scheduled(fixedRate = 60000)
    public void collectMetrics() {
        if (!(dataSource instanceof ShardingSphereDataSource shardingDS)) {
            log.warn("[ShardingMetrics] 当前数据源不是ShardingSphereDataSource，跳过采集");
            return;
        }
        ...
        try {
            ...
            log.error("[ShardingMetrics] 反射字段未找到，可能�ShardingSphere 版本不兼�(5.4.1+)", e);
        } catch (Exception e) {
