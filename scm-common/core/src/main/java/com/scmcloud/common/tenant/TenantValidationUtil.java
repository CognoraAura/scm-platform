...
public class TenantValidationUtil {
     ...
     * @throws BusinessException 如果数据不属于当前租�
     */
    public static void validateDataOwnership(UUID dataTenantId) {
        ...
        if (!currentTenantId.equals(dataTenantId)) {
            log.warn("租户数据访问越权：当前租�{}, 数据租户={}", currentTenantId, dataTenantId);
     ...
     *
     * @param userType 用户类型
     * @return true=平台管理� false=租户用户
     * @deprecated 建议使用无参�isPlatformAdmin() 方法�SecurityContext 获取
     */
    @Deprecated
     ...
     * 2. 用户拥有平台管理员角色（ROLE_PLATFORM_ADMIN �ROLE_SUPER_ADMIN�
     * 3. 租户上下文为 NULL（平台管理员不属于任何租户）
     *
     * @return true=平台管理� false=租户用户或未登录
     */
    public static boolean isPlatformAdmin() {
        try {
            ...
            // 2. 尝试�SecurityContext 获取当前用户（需�SecurityUtils�
            // 注意：为了避免循环依赖，这里使用反射调用 SecurityUtils
            // 如果无法获取到用户信息，则只依赖租户上下文判�
            try {
                ...
                // SecurityUtils 类不存在，回退到基于租户ID的判�
                log.debug("SecurityUtils 类不存在，仅基于租户上下文判断");
            } catch (Exception e) {
                // 反射调用失败，回退到基于租户ID的判�
                log.debug("无法获取用户角色信息，仅基于租户上下文判� {}", e.getMessage());
            }

            // 4. 兜底逻辑：如果无法获取角色信息，租户ID为NULL则认为是平台管理�
            // 这种情况通常发生在系统初始化或特殊场�
            return true;

        } catch (Exception e) {
            log.error("检查平台管理员权限时出错", e);
            return false; // 安全优先，异常情况返�false
        }
    }
     ...
     *
     * @param userType 用户类型
     * @return true=租户管理� false=其他
     * @deprecated 建议使用基于角色的判断方�
     */
    @Deprecated
     ...
     *
     * @param userType 用户类型
     * @return true=有管理权� false=普通用�
     * @deprecated 此方法依赖已废弃的方法，建议使用基于角色的权限检�
     */
    @Deprecated
    ...
    public static void validateRoleAccess(UUID roleTenantId) {
        ...
        if (!currentTenantId.equals(roleTenantId)) {
            log.warn("租户角色访问越权：当前租�{}, 角色租户={}", currentTenantId, roleTenantId);
    ...
    public static void validatePermissionAccess(UUID permissionTenantId) {
        ...
        if (!currentTenantId.equals(permissionTenantId)) {
            log.warn("租户权限访问越权：当前租�{}, 权限租户={}", currentTenantId, permissionTenantId);
     ...
     *
     * @param userType 用户类型
     * @throws BusinessException 如果用户无权创建平台级资�
    ...
    /**
     * 要求当前用户必须是平台管理员，否则抛出异�
     * 这是 isTenantUser() 检查的便捷方法，避免布尔值反转的代码异味
     *
     * @throws BusinessException 如果当前用户不是平台管理�
     */
    ...
    public static boolean allowCrossTenantAccess(String userType, UUID targetTenantId) {
        ...
        // 平台管理员的 tenant_id �NULL，或者目标租户与当前租户不同
     ...
     * @throws BusinessException 如果部门不属于当前租�
     */
    public static void validateDepartmentOwnership(UUID deptTenantId) {
        ...
        if (!currentTenantId.equals(deptTenantId)) {
            log.warn("租户部门访问越权：当前租�{}, 部门租户={}", currentTenantId, deptTenantId);
