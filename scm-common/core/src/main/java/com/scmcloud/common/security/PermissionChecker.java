...
public class PermissionChecker {
    ...
    public boolean hasPermission(UUID userId, String permissionCode) {
        ...
        log.debug("检查用户权� userId={}, permissionCode={}, result={}", userId, permissionCode, hasPermission);
    ...
    public boolean hasRole(UUID userId, String roleCode) {
        ...
        log.debug("检查用户角� userId={}, roleCode={}, result={}", userId, roleCode, hasRole);
        return hasRole;
    }
     ...
     * @param userId 用户 ID
     * @param roleCode 角色编码
     * @throws BusinessException 如果用户无角�
     */
    public void requireRole(UUID userId, String roleCode) {
        if (!hasRole(userId, roleCode)) {
            log.warn("角色检查失� userId={}, roleCode={}", userId, roleCode);
    ...
    public boolean canAccessDepartmentData(UUID userId, UUID userDeptId, UUID targetDeptId,
        ...
        if ("SELF".equals(dataScope)) {
            return false; // 需要在业务层判�create_by
        }
        ...
        // CUSTOM - 自定义规则（需要查�sys_data_permission_rule�
     ...
     *
     * @param userId 用户 ID
     * @param resourceOwnerId 资源所有�ID
     * @param resourceDeptId 资源所属部�ID
     * @param dataScope 数据权限范围
    ...
    public boolean canOperateResource(UUID userId, UUID resourceOwnerId, UUID resourceDeptId, String dataScope) {
        ...
        // DEPT, DEPT_AND_SUB, CUSTOM - 需要结合部门信息判�
     ...
     * 检查用户是否可以分配指定角�
     *
     * @param operatorUserId 操作者用�ID
     * @param operatorRoleLevel 操作者角色等�
